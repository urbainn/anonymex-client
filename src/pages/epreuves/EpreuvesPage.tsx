...
function EpreuvesPage(): ReactElement {
  ...
  const [selected, setSelected] = useState<Record<number, boolean>>({ 1: false, 2: false, 3: false, 4: false, 5: false });
            ...
            {!loading && epreuves.length > 0 && (
              ...
              <Stack direction="row" alignItems="center" spacing={5}>
                    ...
                    {listeDateEpreuves.map((dateEpreuve) => (
                      <Stack>
                        ...
                        <Stack spacing={2} sx={{ pt: 2 }}>
                          {epreuves
                            .filter((epreuve) => memeDate(new Date(epreuve.date), dateEpreuve))
                            .map((epreuve) => (
                ...
                <Stack p={2} flexGrow={1} minWidth={800} spacing={2}>
                  ...
                  {/* Map apres de 1 a 5 */}
                  <EpreuvesFiltres value={3} nombreEpreuves={epreuves.filter(epreuve => epreuve.statut === 3).length} selected={selected[3]} onClick={() => {
                    setSelected({
                      1: false,
                      2: false,
                      3:  !selected[3], 
