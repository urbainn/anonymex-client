    ...
    return (
        <>
            <Modal onClose={onClose} titre={`Supprimer "${session.nom} - ${session.annee}" ?`} newbgcolor="#FFA8A4" width='600px'>
                ...
                <Box display="flex" alignItems="center" flexDirection={'row'} margin={2}>
                    <Checkbox color="error" sx={{ marginRight: 1 }} checked={isChecked} onChange={(e: {target: { checked: boolean }}) => setIsChecked(e.target.checked)}/>
